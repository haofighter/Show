package com.myself.show.show.Ui.baiduMap.test.Utils;
...
public class DrivingRouteOverlay extends OverlayManager {
	...
	public final List<OverlayOptions> getOverlayOptions() {
		...
		if (mRouteLine.getStarting() != null) {//获取起点的信息
			overlayOptionses.add((new MarkerOptions())
					.position(mRouteLine.getStarting().getLocation())
					.icon(getStartMarker() != null ? getStartMarker()
							: BitmapDescriptorFactory
									.fromAssetWithDpi("Icon_start.png"))
					.zIndex(10).title(mRouteLine.getStarting().getTitle()));
		}
	

		if (mRouteLine.getWayPoints() != null) {
			for (int i = 0; i < mRouteLine.getWayPoints().size(); i++) {
				overlayOptionses .add((new MarkerOptions()) .position(
										mRouteLine.getWayPoints().get(i)
												.getLocation())
								.icon(getWayPointsMarker() != null ? getWayPointsMarker()
										: BitmapDescriptorFactory
												.fromAssetWithDpi("Icon_end.png"))
								.zIndex(10)
								.title(mRouteLine.getWayPoints().get(i)
										.getTitle()));
			}
		}
		if (mRouteLine.getTerminal() != null) {//获取终点的信息
			overlayOptionses.add((new MarkerOptions())
					.position(mRouteLine.getTerminal().getLocation())
					.icon(getTerminalMarker() != null ? getTerminalMarker()
							: BitmapDescriptorFactory
									.fromAssetWithDpi("Icon_end.png"))
					.zIndex(10).title(mRouteLine.getTerminal().getTitle()));
		}
	 ...
	 * @return 是否处理了该点击事件
	 */
	public boolean onRouteNodeClick(int i,LatLng mLatLng) {
		if (mRouteLine.getAllStep() != null
	...
	public final boolean onMarkerClick(Marker marker) {
		for (int i = 0; i < mOverlayList.size()-1; i++) {
			...
			Overlay mMarker = mOverlayList.get(i);
